from libs.helpers import auth_required, ResponseType
from db.database import get_user_model, get_event_model
...
def user_stats():
    """devolver cuantos usuarios hay y cuantos eventos hay"""
    total_users = user_model.count()
    total_events = event_model.count()
    return jsonify(
        {
            "type": ResponseType.SUCCESS,
            "message": "Estadísticas obtenidas exitosamente",
            "data": {"total_users": total_users, "total_events": total_events},
        }
    )
...
def update_user(user_id: str, user: dict[str, Any]) -> tuple[Response, int] | Response:
    ...
    if not updated_user:
        return jsonify(
            {"type": ResponseType.ERROR, "message": "Error al actualizar el usuario"}
        ), 500
...
def update_user_bio(user_id: str, user: dict[str, Any]) -> tuple[Response, int] | Response:
    if str(user.get("id")) != user_id:
        return jsonify(
            {"type": ResponseType.DANGER, "message": "Solo puedes actualizar tu propia biografía"}
        ), 403

    data = request.get_json()
    if not data or "bio" not in data:
        return jsonify(
            {"type": ResponseType.WARNING, "message": "El campo 'bio' es requerido"}
        ), 400
    ...
    if not updated_user:
        return jsonify(
            {"type": ResponseType.ERROR, "message": "Error al actualizar la biografía"}
        ), 500
        ...
        return jsonify(
            {
                ...
                "message": "Un administrador no se puede eliminar a sí mismo",
            }
        ), 400
    existing_user = user_model.find_by_id(user_id)
    if not existing_user:
